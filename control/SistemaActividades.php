...
function generateWeekTable($conn) {
    ...
    $horas = array('07:00:00', '08:00:00', '09:00:00', '10:00:00', '11:00:00', '12:00:00', '13:00:00', '14:00:00', '15:00:00', '16:00:00', '17:00:00', '18:00:00', '19:00:00', '20:00:00');
    ...
    foreach ($horas as $hora) {
        ...
        echo '<tr style="border-color: transparent;">';
        echo '<th style="border-color: transparent; font-size: 20px; min-height:100px;">' . substr($hora, 0, 5) . '</th>'; // Encabezado de fila con la hora
        // Generar las celdas para cada día de la semana
        $clone_dt = clone $dt;
        $clone_dt->modify('Monday this week');
        do {
            $fecha = $clone_dt->format('Y-m-d');
            $sql = "SELECT * FROM actividad a
                    INNER JOIN alumnos_actividad al ON a.id_actividad = al.id_EVENTO
                    WHERE a.FECHA = '$fecha' AND a.HORARIO = '$hora' AND al.noCtrl = '$id_usuario'";
            ...
            if (mysqli_num_rows($result) > 0) {
                ...
                while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    echo '<td class="evento-celda" style="background-color: ' . $random_color . '; font-weight: bold; color: white; min-height:100px;" data-id="' . intval($id_actividad) . '">'; // Celda para el evento
                    echo $row['NOMBRE'] . '<br>';
